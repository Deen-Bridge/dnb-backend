...
import spacePollService from "../services/space-poll.service.ts";
import logger from "../config/logger.js";

export const initSpacePollSocket = (io: Server) => {
  const pollNamespace = io.of("/space-polls");

  pollNamespace.on("connection", (socket: Socket) => {
    logger.info(`Socket connected to /space-polls: ${socket.id}`);

    socket.on("join_space_poll_room", (spaceId: string) => {
      const room = `space_poll_${spaceId}`;
      socket.join(room);
      logger.info(`Socket ${socket.id} joined room ${room}`);
    });

    socket.on("leave_space_poll_room", (spaceId: string) => {
      const room = `space_poll_${spaceId}`;
      socket.leave(room);
      logger.info(`Socket ${socket.id} left room ${room}`);
    });

    socket.on("create_poll", async (data: { spaceId: string; hostId: string; question: string; options: string[] }) => {
      try {
    ...
    socket.on("cast_vote", async (data: { pollId: string; userId: string; optionIndex: number }) => {
      try {
    ...
    socket.on("close_poll", async (data: { pollId: string; hostId: string }) => {
      try {
